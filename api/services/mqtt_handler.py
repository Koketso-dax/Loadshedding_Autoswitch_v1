...
class MQTTHandler:
    ...
    def on_connect(self, client: mqtt.Client, userdata: Any, 
        ...
        if rc == 0:
            ...
            logger.error(f"Device {device_id} connection failed with code {rc}")
    ...
    def _process_metric(self, device_id: int, metric_type_name: str, 
        ...
        try:
            ...
            metric_type = MetricType.query.filter_by(name=metric_type_name).first()
            
            if not device or not metric_type:
                logger.error(f"Invalid device {device_id} or metric type {metric_type_name}")
                return
            ...
            if hasattr(current_app, 'metric_queue'):
                ...
                if len(current_app.metric_queue) >= current_app.config.get('MQTT_BATCH_SIZE', 100):
