...
export function useDevices() {
  ...
  const addDevice = async (device_key: string, password: string) => {
    ...
    setError(null);
    try {
      const response = await axios.post<{ message: string; device_id: number }>(
        "/api/devices",
        { device_key, password },
        {
          headers: {
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : "An unknown error occurred while adding the device";
  ...
  const getDevice = async (id: number) => {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : "An unknown error occurred while fetching the device";
      setError(errorMessage);
      throw error;
    } finally {
      setLoading(false);
  ...
  const removeDevice = async (id: number) => {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : "An unknown error occurred while removing the device";
      setError(errorMessage);
      throw error;
    } finally {
      setLoading(false);
